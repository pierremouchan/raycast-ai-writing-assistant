...
export default function Command() {
  ...
  useEffect(() => {
    if (availableActions) {
      if (searchText.length > 0) {
        setFilteredActions(
          Object.keys(availableActions).filter((item) => item.toLowerCase().includes(searchText.toLowerCase())),
        );
      } else {
  ...
  return (
    <List isLoading={!filteredActions} onSearchTextChange={setSearchText}>
